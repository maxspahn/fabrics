...
CONFIG_FILE = "planar_point_config.yaml"
...
def initialize_environment(
    ...
    obstacle_4 = BoxObstacle(name="box_0", content_dict=static_obst_dict)

    env.add_goal(goal)
    #env.add_obstacle(obstacle_1)
    #env.add_obstacle(obstacle_2)
    #env.add_obstacle(obstacle_3)
    env.add_obstacle(obstacle_4)
    obstacles = [obstacle_4]
...
def run_planar_point(n_steps = 1000, render: bool = False):
    ...
    obstacle_4 = obstacles[3]
    planner = set_planner(goal)
    #leaf = planner.get_leaves(['obst_cuboid_0_3_leaf'])[0]
    #leaf.concretize()
    ...
    for i in range(n_steps):
        ...
        obstacle_position = np.array([
            obstacle_4.position()[0],
            obstacle_4.position()[1],
            0,
        ])
        arguments = dict(
            q=q,
            ...
            x_obst_0=obstacle_position,
            sizes_obst_0=obstacle_4.size(),
        )
        #leaf_eval = leaf.evaluate(**arguments)

        #pprint.pprint(leaf_eval['x'])
